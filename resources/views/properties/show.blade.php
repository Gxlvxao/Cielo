        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
                    <a href="{{ route('home') }}" class="hover:text-accent">{{ __('Home') }}</a>
                    <span>/</span>
                    <a href="{{ route('properties.index') }}" class="hover:text-accent">{{ __('Properties') }}</a>
                    <span>/</span>
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="rounded-2xl overflow-hidden shadow-xl aspect-video relative group bg-gray-200">
                        ...
                        @if($property->is_exclusive)
                            <div class="absolute top-4 right-4 bg-accent text-white px-4 py-2 rounded-full font-bold shadow-lg text-sm tracking-wide">{{ __('Exclusive (Off-Market)') }}</div>
                        @endif
                    </div>
                    ...
                    <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                            <div class="p-4 bg-gray-50 rounded-xl">
                                ...
                                <span class="text-xs text-gray-500 uppercase tracking-wide font-medium">{{ __('Bedrooms') }}</span>
                            </div>
                            <div class="p-4 bg-gray-50 rounded-xl">
                                <span class="block text-2xl font-bold text-graphite">{{ $property->bathrooms ?? '-' }}</span>
                                <span class="text-xs text-gray-500 uppercase tracking-wide font-medium">{{ __('Bathrooms') }}</span>
                            </div>
                            <div class="p-4 bg-gray-50 rounded-xl">
                                <span class="block text-2xl font-bold text-graphite">{{ $property->area ?? '-' }}m²</span>
                                <span class="text-xs text-gray-500 uppercase tracking-wide font-medium">{{ __('Living Area') }}</span>
                            </div>
                            <div class="p-4 bg-gray-50 rounded-xl">
                                <span class="block text-xl font-bold text-graphite truncate">{{ $property->price > 0 ? $property->formatted_price : __('Price on Request') }}</span>
                                <span class="text-xs text-gray-500 uppercase tracking-wide font-medium">{{ __('Value') }}</span>
                            </div>
                        </div>
                    </div>

                    <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                        <h3 class="text-2xl font-heading font-bold text-graphite mb-4">{{ __('About this Property') }}</h3>
                ...
                <div class="lg:col-span-1" x-data="{ showModal: false }">
                    <div class="sticky top-24 bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
                        <div class="text-center mb-6">
                            <p class="text-sm text-gray-500 uppercase tracking-wide mb-1">{{ __('Investment') }}</p>
                            <p class="text-4xl font-bold text-graphite font-heading">{{ $property->price > 0 ? $property->formatted_price : __('Price on Request') }}</p>
                        </div>
                                ...
                                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                    <div class="sm:flex sm:items-start">
                                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                                            <h3 class="text-xl font-bold leading-6 text-graphite mb-2" id="modal-title">{{ __('Schedule Visit') }}</h3>
                                            <p class="text-sm text-gray-500 mb-6">{{ __('Preencha seus dados para solicitar uma visita a este imóvel.') }}</p>
                                            ...
                                            <form action="{{ route('properties.visit', $property) }}" method="POST">
                                                ...
                                                <div class="space-y-4">
                                                    ...
                                                    <div>
                                                        <label class="block text-sm font-medium text-gray-700">{{ __('Phone / WhatsApp') }}</label>
                                                    ...
                                                    <div>
                                                        <label class="block text-sm font-medium text-gray-700">{{ __('Preferred Date') }}</label>
                                                    ...
                                                    <div>
                                                        <label class="block text-sm font-medium text-gray-700">{{ __('Message (Optional)') }}</label>
                                                ...
                                                <div class="mt-6 flex flex-row-reverse gap-2">
                                                    <button type="submit" class="inline-flex w-full justify-center rounded-md bg-accent px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-accent/90 sm:w-auto">{{ __('Send Request') }}</button>
                                                    <button type="button" @click="showModal = false" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">{{ __('Cancel') }}</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
