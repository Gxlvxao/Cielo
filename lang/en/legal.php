<?php

return [
    'privacy' => [
        'title' => 'Privacy Policy',
        'content' => '
            <p><strong>Cielo Real Estate</strong> respects the privacy of its users and is committed to protecting the personal data entrusted to us, in compliance with the General Data Protection Regulation (GDPR – EU Regulation 2016/679) and other applicable legislation.</p>
            
            <h3 class="text-xl font-bold mt-6 mb-2">1. Data Controller</h3>
            <p>The entity responsible for processing personal data is Cielo Real Estate, hereinafter referred to as “Cielo”.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">2. Collected Personal Data</h3>
            <p>We may collect and process the following data:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Name</li>
                <li>Email address</li>
                <li>Phone number</li>
                <li>Information provided through contact forms</li>
                <li>Browsing data (cookies, IP, device, browser)</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">3. Purpose of Processing</h3>
            <p>The collected data is intended to:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Respond to contact or information requests</li>
                <li>Provide services and institutional communications</li>
                <li>Comply with legal and regulatory obligations</li>
                <li>Improve user experience on the website</li>
                <li>Send informative communications, when authorized</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">4. Legal Basis</h3>
            <p>Data processing is based on:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>The data subject’s consent</li>
                <li>The execution of pre-contractual diligence</li>
                <li>Compliance with legal obligations</li>
                <li>The legitimate interest of the company</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">5. Data Retention</h3>
            <p>Data will be retained only for the period necessary for the purposes that motivated its collection or as required by law.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">6. Data Sharing</h3>
            <p>Cielo does not sell or transfer personal data to third parties, except when:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Legally obliged</li>
                <li>Necessary for the provision of services (e.g., technology providers), always ensuring confidentiality</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">7. Data Subject Rights</h3>
            <p>The user may, at any time:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Request access, rectification, or deletion of their data</li>
                <li>Withdraw consent</li>
                <li>Request limitation or opposition to processing</li>
                <li>File a complaint with the CNPD – National Data Protection Commission</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">8. Security</h3>
            <p>We adopt appropriate technical and organizational measures to protect data against unauthorized access, loss, or improper disclosure.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">9. Changes</h3>
            <p>Cielo reserves the right to update this Privacy Policy at any time.</p>
        ',
    ],
    'terms' => [
        'title' => 'Terms of Service',
        'content' => '
            <p>By accessing and using the website <strong>cielo.pt</strong>, the user agrees to these Terms of Service.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">1. Object</h3>
            <p>The website is informational and institutional, presenting the activity, vision, and services of Cielo Real Estate.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">2. Website Usage</h3>
            <p>The user commits to:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Use the website lawfully</li>
                <li>Not introduce malicious content</li>
                <li>Not attempt to access restricted areas without authorization</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">3. Content</h3>
            <p>All content provided is merely informative and does not constitute financial, legal, or investment advice.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">4. Intellectual Property</h3>
            <p>All website content (texts, logos, images, design) is the property of Cielo Real Estate or duly authorized third parties and is protected by copyright.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">5. Liability</h3>
            <p>Cielo is not responsible for:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Decisions made based on website information</li>
                <li>Technical failures, interruptions, or errors</li>
                <li>Content of external websites linked to ours</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">6. Changes</h3>
            <p>The company may change these Terms of Service at any time, with changes published on this page.</p>
        ',
    ],
    'cookies' => [
        'title' => 'Cookie Policy',
        'content' => '
            <p>The website <strong>cielo.pt</strong> uses cookies to improve user experience.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">1. What are Cookies</h3>
            <p>Cookies are small text files stored on the user’s device when visiting a website.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">2. Types of Cookies Used</h3>
            <ul class="list-disc ml-6 mt-2">
                <li><strong>Essential cookies:</strong> necessary for the website functioning</li>
                <li><strong>Analytical cookies:</strong> collect anonymous statistical data about site usage</li>
                <li><strong>Functional cookies:</strong> improve user experience</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">3. Purpose</h3>
            <p>Cookies allow to:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Ensure the correct functioning of the website</li>
                <li>Analyze traffic and performance</li>
                <li>Improve content and usability</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">4. Cookie Management</h3>
            <p>The user can, at any time, configure or disable cookies through their browser settings. Disabling may affect website functioning.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">5. Changes</h3>
            <p>This Cookie Policy may be updated at any time.</p>
        ',
    ],
    'notice' => [
        'title' => 'Legal Notice',
        'content' => '
            <h3 class="text-xl font-bold mt-6 mb-2">1. General Information</h3>
            <p>This website is owned by <strong>Cielo Real Estate</strong>.</p>

            <h3 class="text-xl font-bold mt-6 mb-2">2. Nature of Information</h3>
            <p>The information provided:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Has a general and informative character</li>
                <li>Does not constitute an offer, recommendation, or financial advice</li>
                <li>Does not replace consultation with qualified professionals</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">3. Limitation of Liability</h3>
            <p>Cielo is not responsible for:</p>
            <ul class="list-disc ml-6 mt-2">
                <li>Losses or damages resulting from the use of the website</li>
                <li>Incorrect interpretations of the provided information</li>
                <li>Third-party content accessible via links</li>
            </ul>

            <h3 class="text-xl font-bold mt-6 mb-2">4. Applicable Legislation</h3>
            <p>This Legal Notice is governed by Portuguese law.</p>
            <p>For any dispute, the jurisdiction of the legally applicable district in Portugal is competent.</p>
        ',
    ],
];